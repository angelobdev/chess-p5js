...
export default class Piece {
	...
	constructor(symbol: string) {
		...
		let textureFileName = 'assets/' + this._color.toLowerCase() + '_' + this._type.toLowerCase() + '.png';
	...
	public render() {
		if (!this._selected) {
			Chess.p.image(this._texture, this._file * ChessState.TILE_DIMENSION, this._rank * ChessState.TILE_DIMENSION, ChessState.TILE_DIMENSION, ChessState.TILE_DIMENSION);
		}
	}
	...
	public renderFree(x: number, y: number) {
		if (this._selected) {
			...
			this._possibleMoves.forEach((move) => {
				...
				Chess.p.rect(move.toFile * ChessState.TILE_DIMENSION, move.toRank * ChessState.TILE_DIMENSION, ChessState.TILE_DIMENSION, ChessState.TILE_DIMENSION); // Draw overlay rectangle
			});

			// Rendering piece
			Chess.p.image(this._texture, x, y, ChessState.TILE_DIMENSION, ChessState.TILE_DIMENSION);
		}
	}
	...
	private static getPieceColorFromSymbol(symbol: string): PieceColor {
		return symbol === symbol.toUpperCase() ? PieceColor.WHITE : PieceColor.BLACK;
	}
